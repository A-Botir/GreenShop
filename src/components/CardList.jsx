...
const CardList = () => {
  ...
  const calculateTotalPages = () => {
    const itemsPerPage = 9;
    return Math.ceil(flowers.length / itemsPerPage);
  };

  const calculateTotal = () => {
    const itemsPerPage = 9;
    return Math.ceil(
      flowers.filter((flower) => flower.sale === true).length / itemsPerPage,
    );
  };
  ...
  const totalSalePages = calculateTotal();
      ...
      <div className="mb-8 flex items-center justify-between sm:mb-5">
        <nav>
          <Tabs
            ...
            sx={{
              "& .MuiTabs-indicator": {
                backgroundColor: "#46A358 !important",
              },
              "& .MuiTab-root": {
                textTransform: "none",
              },
        ...
        <div className="flex items-center sm:hidden">
          <p className=" text-[15px] md:text-[13px]">Sort by: </p>
          <select
            id="select"
            className="text-[15px] outline-none md:text-[13px]"
          >
      ...
      {value === "two" && (
        <>
          <div className="grid min-h-[1130px] grid-cols-3 justify-between sm:mb-[50px] sm:grid-cols-2 sm:gap-x-5 sm:gap-y-6 md:grid-cols-2 md:gap-x-6 md:gap-y-8 lg:mb-[90px] lg:gap-x-8 lg:gap-y-10">
            ...
            {currentPage === 3 &&
              flowers.slice(18, 20).map((flower) => (
                <div
                  className="card_item transition duration-500"
                  key={flower.id}
                >
                ...
                .map((flower) => (
                  ...
                  >
                    <div className="relative flex items-center justify-center bg-[#FBFBFB] sm:max-h-[200px] sm:rounded-[24px] lg:h-[300px]">
                      ...
                      <p className="absolute left-0 top-3 bg-check px-2 py-[6px] text-[#fff]">
                        13% OFF
                      </p>
                    </div>
                    <p className="py-3 text-[16px] sm:pb-1 sm:pt-2 sm:text-[15px]">
